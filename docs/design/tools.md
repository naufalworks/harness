...
- `path` selects a fixed server from its extension: `.rs` uses `rust-analyzer`; C/C++ sources and headers use `clangd`. The model cannot supply a command. Calls borrow a root-keyed stdio session from a bounded per-registry pool (at most two sessions, 30 seconds idle expiry). Each call resets its deadline, closes the previous document before opening the requested one, and uses only the fixed server/environment contract; protocol errors, timeouts, process exit, and pool eviction shut the session down. A missing or crashed server is `lsp_unavailable`; malformed or oversized protocol data is `lsp_protocol`.
...
Args: `{ operation: "open"|"snapshot"|"click"|"type"|"press"|"screenshot"|"close", url?: string, snapshot_id?: string, ref?: string, text?: string, key?: string, submit?: bool, wait_ms?: int (default 500, max 5000), max_nodes?: int (default 200, max 400), timeout_seconds?: int (default 15, max 30) }`.
...
- `open` accepts only credential-free HTTP(S) URLs. Private, loopback, link-local, metadata and special-use destinations are denied unless the operator explicitly sets `HARNESS_BROWSER_ALLOW_PRIVATE_NETWORK=true`; the same destination check runs after every navigation or interaction, so redirects cannot bypass it. Browser-managed downloads remain disabled and no upload operation is exposed; explicit `upload` and `download` arguments are rejected as `invalid_arguments` before any CDP call. The model cannot choose an executable or endpoint. If the operator supplied `HARNESS_CDP_URL`, it must be a credential-free loopback `ws://` page target. Otherwise the tool discovers `HARNESS_BROWSER_PATH` or a fixed Chrome/Chromium installation, launches headless with an isolated profile and loopback ephemeral debugging port, then selects the page target from `/json/list`. A missing binary/endpoint is `browser_unavailable`.
...
- `Tool::side_effecting_for(args)` is true only for click/type/press. In `ask` mode their permission card names the current URL, snapshot, ref and accessible target (plus the bounded text/key); approval never replays `open`. `open`, `snapshot`, `screenshot`, and `close` never enter the gate. `auto_edit`/`auto_all` may interact automatically.
- `screenshot` calls the fixed `Page.captureScreenshot` method with PNG, viewport-only bounds. The decoded image is capped at 1.5 MiB, must have a PNG signature, and is published atomically below `.harness/artifacts/browser/` with a generated name; the model cannot supply a filesystem path, dimensions, JavaScript, or transfer endpoint. The result reports only the root-relative artifact path, format, byte count, and viewport-only capture policy. Browser-managed downloads and uploads remain disabled.
...
- Structured policy classes are included in permission evidence. Network commands and commands naming protected host/secret paths always require permission even in `auto_all`. Deny-list (always requires permission, even `auto_all`): regexes for `rm -rf /`, `rm -rf ~`, `git push --force`, `git reset --hard`, `mkfs`, `dd if=`, `> /dev/sd`, `chmod -R 777 /`, `curl|sh`, `wget|sh`.
