...
async fn hybrid_recall_uses_offline_vectors_shadowing_usefulness_and_budget() {
    ...
    db.run(|c| {
        let count: i64 = c.query_row("SELECT count(*) FROM memory_embeddings", [], |r| r.get(0))?;
        assert_eq!(count, 3);
...
async fn recall_eval_fixtures_meet_labeled_budgets() {
    ...
    let (mut lex_precision_sum, mut lex_relevant_hit, mut lex_returned) = (0.0f64, 0i64, 0usize);
