...
pub fn parse(args: &Value) -> Result<Ask, String> {
    let description = args.get("description").and_then(Value::as_str).map(str::trim).unwrap_or_default();
    ...
    Ok(Ask {
        description: if description.is_empty() { "explore".to_string() } else { truncate_chars(description, MAX_DESCRIPTION_CHARS) },
...
pub fn definitions(parent_tools: &[Value]) -> Vec<Value> {
    TOOLS.iter()
        .filter_map(|name| parent_tools.iter().find(|t| t["function"]["name"].as_str() == Some(*name)))
        .cloned()
        .collect()
}
...
pub fn record_file(files: &mut Vec<String>, tool: &str, args: &Value) {
    if tool != "read" { return; }
    let Some(path) = args.get("path").and_then(Value::as_str).map(str::trim).filter(|p| !p.is_empty()) else { return };
    if files.iter().any(|seen| seen == path) || files.len() >= MAX_FILES { return; }
    files.push(path.to_string());
}

/// Why a delegation stopped, when it was not the sub-agent's own decision to answer.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Stop { MaxModelCalls, TurnSteps, TurnToolBytes, Deadline, ProviderFailed }
...
impl Stop {
    ...
    pub fn note(self) -> &'static str {
        match self {
            ...
            Self::TurnSteps => "stopped because the turn ran out of steps, so this may be incomplete",
            Self::TurnToolBytes => "stopped because the turn ran out of tool output budget, so this may be incomplete",
            Self::Deadline => "stopped at the turn's time limit, so this may be incomplete",
            Self::ProviderFailed => "stopped because the model call failed, so this may be incomplete",
        }
    }
}
...
pub fn report_content(report: &Report) -> String {
    ...
    let summary = if summary.is_empty() { NO_FINDINGS } else { summary };
    let mut out = format!("sub-agent report ({} model calls, {} tool calls", report.model_calls, report.tool_calls);
    if let Some(stop) = report.stopped { out.push_str(&format!("; {}", stop.note())); }
    out.push_str(")\n");
    out.push_str(&truncate_chars(summary, MAX_SUMMARY_CHARS));
    if !report.files.is_empty() {
        out.push_str("\n\nfiles read:");
        for file in &report.files { out.push_str(&format!("\n- {file}")); }
    }
    out
}

/// One-line label for the step summary and the activity feed.
pub fn label(ask: &Ask) -> String { format!("explore: {}", ask.description) }
...
mod tests {
    ...
    fn a_sub_agent_is_only_ever_offered_read_only_tools() {
        ...
        for name in TOOLS {
            let tool = registry.get(name).expect("a sub-agent tool must exist in the registry");
            assert!(!tool.side_effecting(), "{name} is side-effecting: a sub-agent could then raise an approval");
        }
        assert!(!TOOLS.contains(&"task"), "a sub-agent must not be able to spawn another sub-agent");
    }

    #[test]
    fn definitions_follow_the_parents_exact_schemas() {
        let parent = crate::tools::Registry::standard().schemas().expect("schemas parse");
        let offered = definitions(&parent);
        let names: Vec<&str> = offered.iter().filter_map(|t| t["function"]["name"].as_str()).collect();
        assert_eq!(names, TOOLS);
        let parent_read = parent.iter().find(|t| t["function"]["name"] == "read").unwrap();
        assert_eq!(&offered[0], parent_read, "both agents must describe a tool identically");
        assert!(definitions(&[]).is_empty(), "a turn with no tools cannot offer any to a sub-agent");
    }
    ...
    fn a_prompt_is_required_and_the_label_is_bounded() {
        ...
        let ask = parse(&json!({"description":"D".repeat(200),"prompt":" find it "})).expect("valid");
        assert_eq!(ask.prompt, "find it");
        assert!(ask.description.chars().count() <= MAX_DESCRIPTION_CHARS);
        assert_eq!(parse(&json!({"prompt":"p"})).unwrap().description, "explore");
    }
    ...
    fn the_sub_agent_starts_from_its_own_context_not_the_parents() {
        ...
        assert_eq!(messages.len(), 2, "system prompt and the task, nothing inherited");
        assert_eq!(messages[0]["role"], "system");
        assert!(messages[0]["content"].as_str().unwrap().contains("read-only"));
    ...
    fn only_files_actually_read_are_reported_once_each_and_capped() {
        ...
        for i in 0..40 { record_file(&mut files, "read", &json!({"path":format!("src/f{i}.rs")})); }
        assert_eq!(files.len(), MAX_FILES);
    }

    #[test]
    fn the_report_is_capped_and_says_when_it_is_partial() {
        let full = Report { summary: "y".repeat(5000), files: vec!["src/a.rs".into()], model_calls: 3, tool_calls: 5, stopped: None };
        let content = report_content(&full);
        assert!(content.chars().count() < MAX_SUMMARY_CHARS + 200, "the parent must never receive the transcript");
        assert!(content.contains("files read:") && content.contains("src/a.rs"));
        assert!(!content.contains("incomplete"));

        let partial = Report { summary: String::new(), files: Vec::new(), model_calls: 8, tool_calls: 9, stopped: Some(Stop::MaxModelCalls) };
