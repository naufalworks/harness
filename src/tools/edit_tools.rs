...
use super::{
    content_hash, line_hash, paths, render_line, textdiff, PendingChange, Tool, ToolCtx, ToolResult,
};
...
fn plan_edit(ctx: &ToolCtx, args: &Value) -> Result<PendingChange, ToolResult> {
    ...
    for anchor in anchors {
        let (Some(line), Some(hash)) = (
            anchor.get("line").and_then(Value::as_u64),
            anchor.get("hash").and_then(Value::as_str),
        ) else {
            return Err(ToolResult::err(
                "invalid_arguments",
                "each anchor needs a line number and a hash",
            ));
        };
        if line == 0 {
            return Err(ToolResult::err(
    ...
    if !stale.is_empty() {
        ...
        for i in low..=high {
            detail.push_str(&render_line(i, raw[i - 1]));
            detail.push('\n');
        }
    ...
    if end_line < first || end_line > raw.len() {
        return Err(ToolResult::err(
            "invalid_arguments",
            format!(
                "end_line must be between {first} and {} for this file",
                raw.len()
            ),
        ));
    }
    ...
    let after = match arg_str(args, "old_string") {
        ...
        Some("") => {
            return Err(ToolResult::err(
                "invalid_arguments",
                "old_string must not be empty; omit it to replace whole lines",
            ))
        }
        Some(old) => {
...
fn plan_write(ctx: &ToolCtx, args: &Value) -> Result<PendingChange, ToolResult> {
    ...
    let Some(content) = arg_str(args, "content") else {
        return Err(ToolResult::err("invalid_arguments", "content is required"));
    };
    if content.len() > WRITE_MAX {
        return Err(ToolResult::err(
            "too_large",
            format!(
                "content is {} bytes; the limit is {WRITE_MAX}",
                content.len()
            ),
        ));
    }
            ...
            .unwrap_or(false)
    {
        return Err(ToolResult::err(
            "exists",
            format!("{display} already exists; pass overwrite=true or use edit"),
        ));
    }
...
pub(crate) fn apply(ctx: &ToolCtx, verb: &str, change: PendingChange) -> ToolResult {
    if change.is_noop() {
        return ToolResult::ok(
            format!("{} already had this content", change.display),
            format!(
                "{} is already exactly this content; nothing was written.",
                change.display
            ),
        );
    }
    if let Some(parent) = change.path.parent() {
        if let Err(e) = std::fs::create_dir_all(parent) {
            return ToolResult::err(
                "write_failed",
                format!(
                    "could not create the parent directory of {}: {e}",
                    change.display
                ),
            );
        }
    ...
    let summary = format!(
        "{verb} {} (+{} −{})",
        change.display, change.plus, change.minus
    );
    ...
    for i in low..=high {
        out.push_str(&render_line(i, lines[i - 1]));
        out.push('\n');
    }
...
impl Capped {
    pub fn label(&self) -> String {
        if !self.started {
            return "could not run".into();
        }
        if self.timed_out {
            return format!("timed out after {}s", self.seconds);
        }
        match self.code {
...
pub(crate) fn run_capped(
    ...
    let Ok(out_handle) = std::fs::File::create(&log) else {
...
pub(crate) fn payload(planned: Result<PendingChange, ToolResult>, args: &Value) -> Value {
    match planned {
        Ok(c) => {
            json!({ "path": c.display, "action": c.action, "plus": c.plus, "minus": c.minus, "before_hash": c.before_hash, "after_hash": c.after_hash, "diff": c.diff })
        }
...
impl Tool for Edit {
    ...
    fn schema(&self) -> &'static str {
        include_str!("../../tools/schemas/edit.json")
    }
...
impl Tool for Write {
    ...
    fn schema(&self) -> &'static str {
        include_str!("../../tools/schemas/write.json")
    }
    fn side_effecting(&self) -> bool {
        true
    }
    fn summary(&self, a: &Value) -> String {
        format!("write {}", arg_str(a, "path").unwrap_or("?"))
    }
    fn permission_payload(&self, ctx: &ToolCtx, args: &Value) -> Value {
        payload(plan_write(ctx, args), args)
    }
    fn plan(&self, ctx: &ToolCtx, args: &Value) -> Option<Result<PendingChange, ToolResult>> {
        Some(plan_write(ctx, args))
    }
    fn run(&self, ctx: &ToolCtx, args: Value) -> ToolResult {
        match plan_write(ctx, &args) {
            Ok(change) => apply(ctx, "wrote", change),
            Err(refusal) => refusal,
        }
    }
}
...
mod tests {
    ...
    fn beta_anchor() -> Value {
        json!([{ "line": 2, "hash": line_hash("fn beta() {}") }])
    }
    ...
    #[test]
    fn edit_replaces_whole_lines_and_echoes_fresh_anchors() {
        ...
        assert!(
            out.content.contains(&render_line(2, "fn beta(x: u8) {}")),
            "{}",
            out.content
        );
        assert!(
    ...
    #[test]
    fn edit_refuses_a_stale_anchor_without_writing() {
        let (root, ctx) = project();
        assert_ne!(
            line_hash("fn beta() {}"),
            "0000",
            "the test's deliberately wrong hash must differ from the real one"
        );
        ...
        assert!(
            ...
            "the refusal must hand back current lines: {}",
            out.content
        );
        assert_eq!(
            read_back(&root, "src/lib.rs"),
            LIB,
            "a stale anchor must leave the file untouched"
        );
    ...
    #[test]
    fn edit_requires_old_string_to_occur_exactly_once_in_the_region() {
        let (root, ctx) = project();
    ...
    #[test]
    fn edit_plans_the_change_without_touching_disk() {
        let (root, ctx) = project();
        let planned = Edit.plan(&ctx, &json!({ "path": "src/lib.rs", "anchors": beta_anchor(), "new_string": "fn beta(x: u8) {}" })).unwrap().unwrap();
        assert_eq!(
            (planned.action, planned.plus, planned.minus),
            ("modify", 1, 1)
        );
        assert!(
            planned.diff.contains("+fn beta(x: u8) {}"),
            "{}",
            planned.diff
        );
        assert_eq!(
            planned.before_hash.as_deref(),
            Some(content_hash(LIB).as_str())
        );
    ...
    #[test]
    fn write_creates_refuses_and_overwrites_as_documented() {
        let (root, ctx) = project();
        assert_eq!(
            Write
                .run(&ctx, json!({ "path": "src/lib.rs", "content": "x" }))
                .error_code,
            Some("exists")
        );
        assert_eq!(read_back(&root, "src/lib.rs"), LIB);
        let created = Write.run(
            &ctx,
            json!({ "path": "docs/notes/new.md", "content": "hello\n" }),
        );
        assert_eq!(created.status, ToolStatus::Complete, "{}", created.content);
        assert_eq!(
            read_back(&root, "docs/notes/new.md"),
            "hello\n",
            "parent directories are created inside root"
        );
        assert!(
            matches!(
                created.artifacts.first(),
                Some(Artifact::FileChange {
                    action: "create",
                    before_hash: None,
                    ..
        ...
        let replaced = Write.run(
            &ctx,
            json!({ "path": "docs/notes/new.md", "content": "bye\n", "overwrite": true }),
        );
        assert!(
            matches!(
                replaced.artifacts.first(),
                Some(Artifact::FileChange {
                    action: "modify",
                    ..
                })
        ...
        let repeated = Write.run(
            &ctx,
            json!({ "path": "docs/notes/new.md", "content": "bye\n", "overwrite": true }),
        );
        assert_eq!(repeated.status, ToolStatus::Complete);
        assert!(
            repeated.artifacts.is_empty(),
            "an identical write records no file change: {}",
            repeated.content
        );
        assert_eq!(
            Write
                .run(
                    &ctx,
                    json!({ "path": ".env", "content": "x", "overwrite": true })
                )
                .error_code,
            Some("path_denied")
        );
        assert_eq!(
            Write
                .run(
                    &ctx,
                    json!({ "path": "big.txt", "content": "x".repeat(WRITE_MAX + 1) })
                )
                .error_code,
            Some("too_large")
        );
    ...
    #[test]
    fn edit_appends_diagnostics_and_leaves_no_temp_file_behind() {
        let (root, mut ctx) = project();
        ctx.diagnostics_cmd = Some("echo checked; exit 3".into());
        let out = Edit.run(&ctx, json!({ "path": "src/lib.rs", "anchors": beta_anchor(), "new_string": "fn beta(x: u8) {}" }));
        assert_eq!(out.status, ToolStatus::Complete, "{}", out.content);
        assert!(
            out.content.contains("[diagnostics exit 3]"),
            "{}",
            out.content
        );
        assert!(out.content.contains("checked"), "{}", out.content);
        let leftovers: Vec<String> = std::fs::read_dir(root.join("src"))
            .unwrap()
            .flatten()
            .map(|e| e.file_name().to_string_lossy().to_string())
            .filter(|n| n.contains("harness-tmp"))
            .collect();
        assert!(
            leftovers.is_empty(),
            "atomic write left {leftovers:?} behind"
        );
        std::fs::remove_dir_all(root).ok();
    }

    #[test]
    fn capped_runs_report_a_timeout_instead_of_hanging() {
