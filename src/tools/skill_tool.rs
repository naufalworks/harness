...
impl Tool for Skill {
    ...
    fn schema(&self) -> &'static str {
        include_str!("../../tools/schemas/skill.json")
    }
    ...
    fn run(&self, ctx: &ToolCtx, args: Value) -> ToolResult {
        ...
        if !path.is_file() {
            return ToolResult::err(
                "not_found",
                format!("no skill {name:?} at {relative}. {}", available(ctx)),
            );
        }
        let text = match fs_tools::read_text(&path) {
            Ok(text) => text,
        ...
        if body.is_empty() {
            return ToolResult::err(
                "not_found",
                format!("{relative} has no body below its frontmatter"),
            );
        }
        let bytes = body.len();
...
fn available(ctx: &ToolCtx) -> String {
    ...
    if index.skills.is_empty() {
        return format!(
            "This project has no {}/<name>/SKILL.md files.",
            skills::SKILLS_DIR
        );
    }
...
mod tests {
    ...
    fn project() -> (PathBuf, ToolCtx) {
        let dir =
            std::env::temp_dir().join(format!("harness-skill-tool-{}", crate::storage::uid()));
    ...
    #[test]
    fn skill_loads_one_body_and_names_its_source() {
        let (root, ctx) = project();
        let loaded = Skill.run(&ctx, json!({ "name": " review " }));
        assert_eq!(loaded.status, ToolStatus::Complete);
        assert!(
            loaded.content.contains("source: skills/review/SKILL.md"),
            "{}",
            loaded.content
        );
        assert!(
            loaded.content.contains("Re-anchor before every edit."),
            "{}",
            loaded.content
        );
        assert!(
            !loaded.content.contains("description: How this repo"),
            "frontmatter is index data, not body: {}",
            loaded.content
        );
        assert!(
            loaded.content.contains("cannot grant tool permissions"),
            "{}",
            loaded.content
        );
        assert!(
            loaded.summary.starts_with("skill review ("),
            "{}",
            loaded.summary
        );
        assert!(
            !Skill.side_effecting(),
            "loading a document must never wait for an approval"
        );
        std::fs::remove_dir_all(root).ok();
    }

    #[test]
    fn skill_caps_a_long_body_and_says_where_the_rest_is() {
        let (root, ctx) = project();
        ...
        assert!(
            capped
                .content
                .contains("truncated at 16384 bytes; read skills/huge/SKILL.md"),
            "{}",
            capped.content
        );
        assert!(
            capped.content.len() < skills::MAX_BODY_BYTES + 512,
            "{} bytes",
            capped.content.len()
        );
        assert!(
            capped.summary.ends_with(", truncated)"),
            "{}",
            capped.summary
        );
        std::fs::remove_dir_all(root).ok();
    ...
    #[test]
    fn skill_refuses_anything_that_is_not_a_known_skill_directory() {
        let (root, ctx) = project();
        for name in [
            json!("../secrets"),
            json!("review/../.."),
            ...
            assert!(
                refused.content.contains("review"),
                "a refusal lists usable names: {}",
                refused.content
            );
        }
        ...
        assert!(
            missing.content.contains("skills/deploy/SKILL.md"),
            "{}",
            missing.content
        );
        assert!(
            missing.content.contains("Available: huge, review"),
            "{}",
            missing.content
        );
        std::fs::remove_dir_all(root).ok();
