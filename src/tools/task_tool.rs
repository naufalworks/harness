...
impl Tool for Task {
    ...
    fn schema(&self) -> &'static str {
        include_str!("../../tools/schemas/task.json")
    }
    ...
    fn summary(&self, args: &Value) -> String {
        let label = args
            ...
            .unwrap_or_default();
        if label.is_empty() {
            "explore (sub-agent)".to_string()
        } else {
            format!("explore: {}", truncate_chars(label, SUMMARY_MAX))
        }
    }
    fn run(&self, _ctx: &ToolCtx, _args: Value) -> ToolResult {
        ToolResult::err(
            "internal_error",
            "task is dispatched by the agent loop, not the tool registry",
        )
    }
}
...
mod tests {
    ...
    fn the_label_is_bounded_and_the_prompt_never_leaks_into_it() {
        let summary =
            Task.summary(&json!({ "description": "x".repeat(200), "prompt": "find the secret" }));
        assert!(summary.starts_with("explore: "));
        assert!(summary.chars().count() <= "explore: ".chars().count() + SUMMARY_MAX);
        assert!(
            !summary.contains("find the secret"),
            "the permission prompt must not carry model text for another model"
        );
        assert_eq!(
            Task.summary(&json!({ "prompt": "p" })),
            "explore (sub-agent)"
        );
    }
    ...
    fn a_direct_registry_invocation_refuses_instead_of_pretending_to_explore() {
        let result = Task.run(
            &ctx(),
            json!({ "description": "where is X handled", "prompt": "look" }),
        );
        assert_eq!(result.error_code, Some("internal_error"));
        assert!(
            !Task.side_effecting(),
            "a read-only sub-agent must never raise an approval"
        );
    }
}
