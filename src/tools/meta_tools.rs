...
impl Tool for Think {
    ...
    fn schema(&self) -> &'static str {
        include_str!("../../tools/schemas/think.json")
    }
    ...
    fn run(&self, _ctx: &ToolCtx, args: Value) -> ToolResult {
        ...
        let Some(thought) = thought else {
            return ToolResult::err(
                "invalid_arguments",
                "thought is required and must not be blank",
            );
        };
...
impl Tool for TodoWrite {
    ...
    fn schema(&self) -> &'static str {
        include_str!("../../tools/schemas/todo_write.json")
    }
    fn side_effecting(&self) -> bool {
        false
    }
    fn summary(&self, args: &Value) -> String {
        let Ok(items) = normalize(args) else {
            return "plan rejected".into();
        };
        ...
        match items.iter().find(|(_, status)| status == "in_progress") {
            Some((text, _)) => format!(
                "plan {done}/{} · {}",
                items.len(),
                truncate_chars(text, CURRENT_MAX)
            ),
            None => format!("plan {done}/{} done", items.len()),
        }
    }
    fn run(&self, _ctx: &ToolCtx, args: Value) -> ToolResult {
        let items = match normalize(&args) {
            Ok(items) => items,
...
fn normalize(args: &Value) -> Result<Vec<(String, String)>, ToolResult> {
    ...
    let Some(raw) = args.get("items").and_then(Value::as_array) else {
        return Err(invalid(
            "items must be an array of {text, status} objects".into(),
        ));
    };
    if raw.len() > MAX_PLAN_ITEMS {
        return Err(invalid(format!(
            "a plan holds at most {MAX_PLAN_ITEMS} items, got {}",
            raw.len()
        )));
    }
    ...
    for (i, item) in raw.iter().enumerate() {
        ...
        let text = item
            ...
            .unwrap_or_default();
        if text.is_empty() {
            return Err(invalid(format!("item {seq} needs text")));
        }
        if text.chars().count() > MAX_PLAN_TEXT {
            return Err(invalid(format!(
                "item {seq} is longer than {MAX_PLAN_TEXT} characters"
            )));
        }
        let status = item
            .get("status")
        ...
        if !PLAN_STATUSES.contains(&status) {
            return Err(invalid(format!(
                "item {seq} has status {status:?}; expected one of {}",
                PLAN_STATUSES.join(", ")
            )));
        }
...
mod tests {
    ...
    fn think_records_its_text_and_leaves_the_plan_alone() {
        ...
        let r = Think.run(
            &ctx,
            json!({ "thought": "  two problems: the anchor drifts, and grep is slow  " }),
        );
        assert_eq!(r.status, ToolStatus::Complete);
        assert_eq!(r.summary, "noted");
        assert_eq!(
            r.content,
            "two problems: the anchor drifts, and grep is slow"
        );
        assert!(r.artifacts.is_empty(), "think must not touch the plan");
        assert!(
            !Think.side_effecting(),
            "a scratchpad must never wait for an approval"
        );
        assert_eq!(
            Think.run(&ctx, json!({ "thought": "   " })).error_code,
            Some("invalid_arguments")
        );
        assert_eq!(
            Think.run(&ctx, json!({})).error_code,
            Some("invalid_arguments")
        );
        assert_eq!(
            Think
                .run(&ctx, json!({ "thought": "x".repeat(THOUGHT_MAX + 1) }))
                .error_code,
            Some("too_large")
        );
    }
    ...
    fn todo_write_normalizes_and_hands_the_plan_to_the_loop() {
        ...
        assert_eq!(
            body["items"][0]["text"], "read the failing test",
            "text is trimmed"
        );
        assert_eq!(
            body["items"][2]["status"], "pending",
            "an absent status defaults to pending"
        );
        assert_eq!(
            body["items"][2]["seq"], 3,
            "seq is the position, so the UI can render the order"
        );
    ...
    fn todo_write_refuses_a_plan_it_could_not_store() {
        ...
        let too_many: Vec<Value> = (0..=MAX_PLAN_ITEMS)
        ...
        for args in [
            ...
            assert!(
                ...
                "a refused plan must not be handed to the loop"
            );
        }
    }
