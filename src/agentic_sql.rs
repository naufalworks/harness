...
pub const STEP_NEXT_SEQ: &str =
    r#"SELECT COALESCE(MAX(seq),-1)+1 FROM turn_steps WHERE request_id=?1"#;
...
pub const PERMISSION_RESOLVE: &str =
    r#"UPDATE permission_requests SET status=?2,resolved_at=?3 WHERE id=?1 AND status='pending'"#;
...
pub const FILE_CHANGE_REVERTED: &str =
    r#"UPDATE file_changes SET reverted_at=?2 WHERE id=?1 AND reverted_at IS NULL"#;

pub const PLAN_CLEAR: &str = r#"DELETE FROM plan_items WHERE session_id=?1"#;
pub const PLAN_INSERT: &str =
    r#"INSERT INTO plan_items(id,session_id,seq,text,status,updated_at) VALUES(?1,?2,?3,?4,?5,?6)"#;
pub const PLAN_LIST: &str =
    r#"SELECT seq,text,status,updated_at FROM plan_items WHERE session_id=?1 ORDER BY seq"#;
...
pub const RECOVER_STEPS: &str =
    r#"UPDATE turn_steps SET status='interrupted',finished_at=?1 WHERE status='running'"#;
pub const RECOVER_PERMISSIONS: &str =
    r#"UPDATE permission_requests SET status='expired',resolved_at=?1 WHERE status='pending'"#;
