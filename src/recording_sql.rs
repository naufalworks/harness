...
pub const EVENT: &str =
    r#"INSERT INTO recording_events(request_id,kind,created_at) VALUES(?1,?2,?3)"#;
...
pub const COMPLETE_USER: &str =
    r#"UPDATE messages SET status='complete' WHERE id=?1 AND status='pending'"#;
...
pub const FAIL_MESSAGE: &str =
    r#"UPDATE messages SET status='failed' WHERE id=?1 AND status='pending'"#;
...
pub const LINK_JOB: &str =
    r#"UPDATE recording_outbox SET job_id=?2 WHERE request_id=?1 AND job_id IS NULL"#;
