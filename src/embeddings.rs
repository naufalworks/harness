...
fn hash(bytes:&[u8])->u64{
    ...
    for byte in bytes {value^=*byte as u64;value=value.wrapping_mul(0x100000001b3);}
    value
}
...
pub fn embed(text:&str)->Vec<f32>{
    ...
    let tokens=normalized.split(|c:char|!c.is_alphanumeric()).filter(|v|!v.is_empty()).collect::<Vec<_>>();
    ...
    for token in &tokens {
        ...
        for n in 2..=3 {
            ...
            for gram in chars.windows(n){add(&mut vector,&format!("g{}:{}",n,gram.iter().collect::<String>()),0.35);}
        }
    }
    for pair in tokens.windows(2){add(&mut vector,&format!("b:{} {}",pair[0],pair[1]),0.5);}
...
pub fn decode(bytes:&[u8],dimensions:usize)->Option<Vec<f32>>{
    if dimensions!=DIMENSIONS || bytes.len()!=dimensions*4{return None;}
...
mod tests{
    use super::*;
    #[test]fn vectors_are_deterministic_normalized_and_round_trip(){
        let first=embed("Rust memory engine");let second=embed("Rust memory engine");
        assert_eq!(first,second);assert!((cosine(&first,&first)-1.0).abs()<0.0001);
        assert_eq!(decode(&encode(&first),DIMENSIONS).unwrap(),first);
    }
    #[test]fn character_features_recall_related_spelling(){
        ...
        assert!(related>unrelated && related>0.05,"related={related} unrelated={unrelated}");
    }
}
