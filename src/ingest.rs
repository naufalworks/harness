...
pub fn sanitized_source(text: &str, format: &str) -> String {
        ...
        match value {
            ...
            Value::Object(items) => {
                for (key, item) in items {
                    if crate::safety::sensitive(key) {
                        *item = Value::String("[REDACTED SENSITIVE CONTENT]".into());
                    } else {
                        clean(item);
                    }
                }
    ...
    if format == "junie" {
        return crate::safety::redact(text);
    }
    text.split('\n')
        .map(|line| {
        ...
        None => {
            ...
            for line in text.lines().take(32) {
                if let Ok(v) = serde_json::from_str::<Value>(line) {
                    ...
                    if ["session_meta", "response_item", "turn_context"].contains(&t) {
                        detected = Some("codex");
                        break;
                    }
                    if v.get("sessionId").is_some() {
                        detected = Some("claude");
                        break;
                    }
                    if ["title", "session", "model_change", "message"].contains(&t) {
                        detected = Some("omp");
                        break;
                    }
                    if ["user", "assistant"].contains(&t) && v.get("message").is_some() {
                        detected = Some("claude");
                        break;
                    }
                }
            ...
            detected
                .ok_or_else(|| {
                    anyhow::anyhow!("cannot detect import format; specify it explicitly")
                })?
                .to_string()
        }
    };
    ...
    if format == "junie" {
        ...
        for line in text.lines() {
            ...
            if let Some(next) = header {
                if !role.is_empty() {
                    push(role.clone(), std::mem::take(&mut buffer));
                }
                role = next.to_string();
        ...
        for (line_no, line) in text.lines().enumerate() {
            ...
            let v: Value = match serde_json::from_str(line) {
                ...
                Err(_) => {
                    warnings.push(format!(
                        "line {}: malformed JSON retained in source",
                        line_no + 1
                    ));
                    continue;
                }
            ...
            if let Some(m) = message {
                if let (Some(role), Some(body)) =
                    (m.get("role").and_then(Value::as_str), m.get("content"))
                {
                        ...
                        if let Value::Array(blocks) = body {
                            for block in blocks {
                                if matches!(
                                    ...
                                    push("tool".into(), block.to_string());
                                }
                            }
                        }
                        continue;
        ...
        if skipped > 0 {
            warnings.push(format!(
                "{skipped} metadata/unsupported records retained in source but not normalized"
            ));
        }
    }
    if events.is_empty() {
        bail!("no normalizable messages; import was not accepted");
    }
...
pub fn chunks(events: &[Event]) -> Vec<Vec<Event>> {
    ...
    for event in events {
        ...
        while start < event.content.len() {
            ...
            if size + body.len() > CHUNK_BYTES && !current.is_empty() {
                chunks.push(std::mem::take(&mut current));
                size = 0;
            }
            size += body.len();
            current.push(Event {
                id: format!("{}:part-{part}", event.id),
...
mod tests {
    use super::*;
    #[test]
    fn retains_consecutive_messages_and_markup() {
        ...
        assert!(events[0].content.starts_with('<'));
    }
    #[test]
    fn unicode_chunks_round_trip_past_forty_messages() {
        let events = (0..100)
            .map(|i| Event {
                id: i.to_string(),
                role: "user".into(),
                content: "🦀Halo".repeat(4000),
            })
            .collect::<Vec<_>>();
        ...
        assert!(out
            .iter()
            .all(|c| c.iter().map(|e| e.content.len()).sum::<usize>() <= CHUNK_BYTES));
    }
    #[test]
    fn claude_tool_results_are_not_user_evidence() {
    ...
    #[test]
    fn sanitized_jsonl_stays_parseable() {
        let text =
            r#"{"type":"user","message":{"role":"user","content":"api_key=synthetic-value"}}"#;
        let clean = sanitized_source(text, "claude");
        assert!(serde_json::from_str::<Value>(&clean).is_ok());
        assert!(!clean.contains("synthetic-value"));
    }
    #[test]
    fn junie_keeps_normal_markdown_headings() {
        let (_, events, _) = parse(
            "## User\nquestion\n## Details\nmore\n## Assistant\nanswer",
            Some("junie"),
        )
        .unwrap();
        assert_eq!(events.len(), 2);
        assert!(events[0].content.contains("## Details"));
    }
}
