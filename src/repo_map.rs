...
fn symbol_name(line: &str) -> Option<String> {
    if line.len() > 500 || line.starts_with(char::is_whitespace) {
        return None;
    }
...
fn fallback_symbols(root: &Path, files: &[PathBuf]) -> BTreeMap<PathBuf, Vec<String>> {
    ...
    for rel in files {
        ...
        if fs::metadata(&path)
            .ok()
            .is_none_or(|m| m.len() > MAX_SYMBOL_FILE_BYTES)
        {
            continue;
        }
        ...
        if !symbols.is_empty() {
            result.insert(rel.clone(), symbols);
        }
    }
    result
}
...
fn ctags_symbols(root: &Path, files: &[PathBuf]) -> Option<BTreeMap<PathBuf, Vec<String>>> {
    ...
    for line in text.lines() {
        ...
        if symbols.len() < 24 && !symbols.iter().any(|v| v == fields[0]) {
...
fn bounded(signature: &str, files: &[PathBuf], symbols: &BTreeMap<PathBuf, Vec<String>>) -> String {
    ...
    for rel in files {
        ...
        if let Some(names) = symbols.get(rel) {
            line.push_str(" :: ");
        ...
        if out.len() + line.len() > MAX_REPO_MAP_BYTES {
            ...
            while out.len() + marker.len() > MAX_REPO_MAP_BYTES {
                if out.pop().is_none() {
                    break;
                }
            }
...
pub fn load_or_refresh(root: &Path) -> Result<RepoMap> {
    ...
    if let Ok(text) = fs::read_to_string(&cache) {
        if text
            .lines()
            .next()
            .is_some_and(|line| line.ends_with(&format!("signature:{signature}")))
            && text.len() <= MAX_REPO_MAP_BYTES
        {
            return Ok(RepoMap {
                id,
                text,
...
mod tests {
    ...
    fn project() -> PathBuf {
        ...
        fs::write(root.join("target/generated.rs"), "fn hidden(){}\n").unwrap();
        fs::canonicalize(root).unwrap()
    }
    #[test]
    fn builds_a_bounded_secret_free_symbol_map_and_caches_it() {
        let root = project();
        ...
        let second = load_or_refresh(&root).unwrap();
        ...
        .unwrap();
        let third = load_or_refresh(&root).unwrap();
        assert!(third.refreshed && third.text.contains("restart"));
        fs::remove_dir_all(root).ok();
    }
    #[test]
    fn truncates_only_at_utf8_boundaries() {
        let files = (0..2000)
            .map(|i| PathBuf::from(format!("資料/{i:04}-{}.rs", "長".repeat(8))))
            .collect::<Vec<_>>();
    ...
    #[cfg(unix)]
    #[test]
    fn git_discovery_rejects_tracked_or_untracked_symlinks() {
        let root = project();
