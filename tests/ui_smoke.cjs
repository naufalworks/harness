...
(async()=>{
 ...
 try{
  ...
  const agentSteps=[
   {id:'synthetic-step',seq:0,kind:'model_call',status:'complete',tool_name:null,tool_call_id:null,summary:null,input_preview:null,output_preview:null,preview_visibility:'private_hidden',previews_capped:false,output_bytes:0,truncated:false,tokens_in:11,tokens_out:7,error_code:null,started_at:'2026-09-09T05:41:00Z',finished_at:'2026-09-09T05:41:01Z'},
   {id:'synthetic-think',seq:1,kind:'tool_call',status:'complete',tool_name:'think',tool_call_id:'call-think',summary:'noted',input_preview:null,output_preview:null,preview_visibility:'private_hidden',previews_capped:false,output_bytes:0,truncated:false,tokens_in:null,tokens_out:null,error_code:null,started_at:'2026-09-09T05:41:01Z',finished_at:'2026-09-09T05:41:02Z'},
   {id:'synthetic-verify',seq:2,kind:'verification',status:'complete',tool_name:null,tool_call_id:null,summary:null,input_preview:null,output_preview:null,preview_visibility:'private_hidden',previews_capped:false,output_bytes:0,truncated:false,tokens_in:null,tokens_out:null,error_code:null,started_at:'2026-09-09T05:41:02Z',finished_at:'2026-09-09T05:41:03Z'}
  ];
  ...
  await page.route('http://127.0.0.1:8080/**',async route=>{
   ...
   if(p==='/health')result={ready:true,commit:'__HARNESS_BUILD_COMMIT__',binary_sha256:'0'.repeat(64),schema_version:6,database:{ready:true},workers:{recording:true,extraction:true}};
    ...
    receipt={request_id:body.request_id,session_id:body.session_id,scope:body.scope,state:'complete',model:'synthetic-main',provider_id:'environment',provider_version:7,captured_at:'2026-09-09T05:41:00Z',updated_at:'2026-09-09T05:41:03Z',response:chatHistory[1].content,recalled:[{...data,revision:2}],redacted:false,memory_status:'pending',events:[]};result=receipt;
  ...
  assert.strictEqual(await page.locator('#activity-provider').innerText(),'environment');assert.strictEqual(await page.locator('#activity-model').innerText(),'synthetic-main');assert.strictEqual(await page.locator('#activity-routing').innerText(),'Pinned provider version 7');assert.strictEqual(await page.locator('#activity-phase').innerText(),'Complete');assert((await page.locator('#activity-elapsed').innerText()).includes('elapsed'));
  const activityText=await page.locator('#agent-turn').innerText();assert(activityText.includes('Private model reasoning and scratchpad text are not displayed.'));const thinkText=await page.locator('.agent-step').filter({hasText:'think'}).textContent();assert(thinkText.includes('Private scratchpad content is intentionally hidden.'));assert(!thinkText.includes('PRIVATE-SCRATCHPAD-TEXT'));assert(!activityText.includes('PRIVATE-SCRATCHPAD-TEXT'));
