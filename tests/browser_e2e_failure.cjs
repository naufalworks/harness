...
function launch(env) { const child = spawn(binary, [], { cwd: root, env: { ...process.env, ...env }, stdio: ['ignore', 'pipe', 'pipe'] }); let out = '', err = ''; child.stdout.on('data', x => { out += x; }); child.stderr.on('data', x => { err += x; }); child.diag = () => `stdout:\n${out}\nstderr:\n${err}`; return child; }
