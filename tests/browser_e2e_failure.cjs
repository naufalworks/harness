// Failure-path E2E coverage. No API routes are mocked: Chromium, axum, SQLite,
// filesystem, and the loopback provider are all real.
...
const token = () => 'e2e-failure-' + crypto.randomBytes(24).toString('hex');
...
function json(res, status, body) { const b = Buffer.from(JSON.stringify(body)); res.writeHead(status, { 'content-type': 'application/json', 'content-length': b.length }); res.end(b); }
function providerServer(mode) {
  const calls = [];
  const server = http.createServer((req, res) => {
    if (req.method === 'GET' && req.url === '/v1/models') return json(res, 200, { data: [{ id: 'e2e-model' }] });
    ...
    let raw = ''; req.on('data', chunk => { raw += chunk; }); req.on('end', () => {
      ...
      else if (mode === 'crash' && !tools.some(m => m.tool_call_id === 'bash-1')) reply = { role: 'assistant', content: null, tool_calls: [{ id: 'bash-1', type: 'function', function: { name: 'bash', arguments: JSON.stringify({ command: "printf 'verified\\n'", description: 'Verify the edit' }) } }] };
      else reply = { role: 'assistant', content: mode === 'deny' ? 'The edit was denied and the file was left unchanged.' : 'Changed beta to gamma and verified it.' };
      json(res, 200, { choices: [{ message: reply }], usage: { prompt_tokens: 11, completion_tokens: 7 } });
    });
  });
  return new Promise(resolve => server.listen(0, '127.0.0.1', () => resolve({ server, calls, port: server.address().port })));
}
function launch(env, cwd = root) { const child = spawn(binary, [], { cwd, env: { ...process.env, ...env }, stdio: ['ignore', 'pipe', 'pipe'] }); let out = '', err = ''; child.stdout.on('data', x => { out += x; }); child.stderr.on('data', x => { err += x; }); child.diag = () => `stdout:\n${out}\nstderr:\n${err}`; return child; }
async function stop(child, signal = 'SIGTERM') { if (!child || child.exitCode !== null) return; child.kill(signal); await Promise.race([new Promise(resolve => child.once('exit', resolve)), wait(3000).then(() => child.kill('SIGKILL'))]); }
...
async function boot(project, db, auth, provider) { const port = await new Promise(resolve => { const s = http.createServer().listen(0, '127.0.0.1', function () { const p = this.address().port; this.close(() => resolve(p)); }); }); const app = launch({ HARNESS_ADDR: `127.0.0.1:${port}`, HARNESS_DB: db, HARNESS_AUTH_TOKEN: auth, HARNESS_API_KEY: 'e2e-provider-key', HARNESS_BASE_URL: `http://127.0.0.1:${provider.port}/v1`, HARNESS_MODEL: 'e2e-model' }, path.dirname(db)); try { await waitFor(async () => { try { return (await fetch(`http://127.0.0.1:${port}/memory/status`, { headers: { Authorization: `Bearer ${auth}` } })).ok; } catch { return false; } }, 30000, 'Rust readiness'); } catch (error) { throw new Error(`${error.message}\n${app.diag()}`); } return { app, port }; }
...
async function crashRecovery() {
  const auth = token(), fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'harness-crash-')), dbDir = fs.mkdtempSync(path.join(os.tmpdir(), 'harness-crash-db-')), project = path.join(fixture, 'project'), db = path.join(dbDir, 'harness.db'); fs.mkdirSync(project); fs.writeFileSync(path.join(project, 'notes.md'), 'alpha\nbeta\n'); const provider = await providerServer('crash'); let browser, app, appPort;
