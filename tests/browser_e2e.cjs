...
async function main() {
  ...
  const app = launch(binary, { HARNESS_ADDR: `127.0.0.1:${appPort}`, HARNESS_DB: db, HARNESS_AUTH_TOKEN: token, HARNESS_API_KEY: 'e2e-provider-key', HARNESS_BASE_URL: `http://127.0.0.1:${provider.port}/v1`, HARNESS_MODEL: 'e2e-model', HARNESS_PROVIDER_ALLOW_LOOPBACK_HTTP: '1' }, dbDir); let browser;
  let page;
  try {
