...
async function main() {
  ...
  const app = launch(binary, { HARNESS_ADDR: `127.0.0.1:${appPort}`, HARNESS_DB: db, HARNESS_AUTH_TOKEN: token, HARNESS_API_KEY: 'e2e-provider-key', HARNESS_BASE_URL: `http://127.0.0.1:${provider.port}/v1`, HARNESS_MODEL: 'e2e-model', HARNESS_PROVIDER_ALLOW_LOOPBACK_HTTP: '1', HARNESS_PROJECT_BROWSE_ROOTS: fixture }, dbDir); let browser;
  let page;
  try {
    ...
    assert(!(await page.locator('body').innerText()).includes(uiProviderKey));
    const storageDump = await page.evaluate(() => JSON.stringify({...localStorage, ...sessionStorage})); assert(!storageDump.includes(uiProviderKey));
    ...
    let environmentCard = page.locator('.provider-card').filter({hasText:'environment'}); await environmentCard.getByRole('button', {name:'Select'}).click();
    await page.fill('#rootpath', '/typed/draft-that-must-survive'); await page.selectOption('#permissionmode', 'auto_edit'); await page.click('#folder-open'); await page.waitForFunction(() => document.querySelectorAll('#folder-list .folder-entry').length === 1);
    await page.evaluate(outside => browseProjectDirectories(outside), path.dirname(fixture)); await page.waitForFunction(() => document.querySelector('#folder-browser-status')?.textContent.includes('Could not browse'));
    assert.strictEqual(await page.locator('#rootpath').inputValue(), '/typed/draft-that-must-survive', 'browse denial must preserve the typed draft'); assert.strictEqual(await page.locator('#permissionmode').inputValue(), 'auto_edit', 'browsing must never change permission mode');
    await page.click('#folder-cancel'); assert.strictEqual(await page.locator('#rootpath').inputValue(), '/typed/draft-that-must-survive', 'cancel must preserve the pre-browser typed draft'); assert.strictEqual(await page.locator('#permissionmode').inputValue(), 'auto_edit');
    await page.click('#folder-open'); await page.waitForFunction(() => document.querySelectorAll('#folder-list .folder-entry').length === 1); await page.locator('#folder-list .folder-entry').first().click(); await page.waitForFunction(() => document.querySelector('#folder-current')?.textContent.length > 0); await page.locator('#folder-list .folder-entry').filter({hasText:'project'}).click(); await page.waitForFunction(project => document.querySelector('#folder-current')?.textContent === project, project); await page.click('#folder-use');
    assert.strictEqual(await page.locator('#rootpath').inputValue(), project, 'Use this folder copies only the selected path into the draft'); assert.strictEqual(await page.locator('#permissionmode').inputValue(), 'auto_edit', 'choosing a folder must not grant or alter permission');
    await page.selectOption('#permissionmode', 'ask'); await page.fill('#diagnosticscmd', 'grep -q gamma notes.md'); await page.click('#projectform button[type="submit"]'); await page.waitForTimeout(1000); const savedState = await page.evaluate(() => ({ notice: document.querySelector('#notice').textContent, root: document.querySelector('#rootpath').value })); assert(savedState.notice.includes('saved'), JSON.stringify(savedState)); assert.strictEqual(savedState.root, project);
    ...
    await page.waitForFunction(() => document.querySelector('#activity-provider')?.textContent === 'environment'); assert.strictEqual(await page.locator('#activity-model').innerText(), 'manual-e2e-model'); assert((await page.locator('#activity-routing').innerText()).includes('Pinned provider version')); assert.strictEqual(await page.locator('#activity-phase').innerText(), 'Waiting for approval'); assert((await page.locator('#agent-runtime').innerText()).includes('Private model reasoning and scratchpad text are not displayed.'));
    // Provider selection is mutable configuration for future work only. Change it while this
    // turn is paused on approval; the admitted turn must remain pinned to environment v1.
    await page.evaluate(() => api('/providers/e2e-custom/select', {})); await page.waitForFunction(() => document.querySelector('#activity-provider')?.textContent === 'environment'); assert.strictEqual(await page.locator('#activity-model').innerText(), 'manual-e2e-model', 'provider switch must not reassign an admitted turn model');
    ...
    const pinnedRows = JSON.parse(querySqlite(db, 'SELECT DISTINCT provider_id,provider_version FROM chat_receipts WHERE state IN ("complete","generating");')); assert(pinnedRows.some(row => row.provider_id === 'environment' && row.provider_version === 1), JSON.stringify(pinnedRows));
    await page.click('[data-view="settings"]'); environmentCard = page.locator('.provider-card').filter({hasText:'environment'}); await environmentCard.getByRole('button', {name:'Select'}).click(); customCard = page.locator('.provider-card').filter({hasText:'e2e-custom'}); page.once('dialog', dialog => dialog.accept()); await customCard.getByRole('button', {name:'Delete'}).click(); await page.waitForFunction(() => !document.querySelector('#provider-list')?.textContent.includes('e2e-custom'));
