...
/// POST /chat/stream — same memory pipeline as /chat, but the main agent's
...
async fn chat_stream(
    ...
    let stream = async_stream::stream! {
        macro_rules! emit {
            ($v:expr) => {
                yield Ok::<Event, Infallible>(Event::default().data(serde_json::to_string(&$v).unwrap()))
            };
        }
        ...
        if prompt.starts_with("confirm ") || prompt.starts_with("reject ") {
            ...
            if parts.len() >= 2 {
                ...
                if let Ok(true) = h.store.resolve_confirmation(parts[1], is_confirm) {
                    ...
                    emit!(json!({"type":"done","session_id":session_id,"response":msg,"confirmation_prompt":null,"recalled_context_applied":false,"background_status":"synced"}));
                    return;
                }
            }
        }
        ...
        emit!(json!({"type":"done","session_id":session_id,"confirmation_prompt":gatekeeper_prompt,"recalled_context_applied":context_applied,"background_status":"synced"}));
    };
